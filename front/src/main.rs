...
fn films_endpoint() -> String {
    ...
    let protcol = location.protocol().expect("should have a protocol");
    let endpoint = format!("{}://{}/{}", protcol, host, API_ENDPOINT);
...
async fn get_films() -> Vec<Film> {
    ...
    reqwest::get(&films_endpoint())
        .await
        .expect("Failed to fetch films")
        .json::<Vec<Film>>()
        .await
        .expect("Failed to parse films")
}
